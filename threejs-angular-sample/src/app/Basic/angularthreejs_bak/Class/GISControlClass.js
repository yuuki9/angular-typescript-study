...
class GISControlClass {
     ...
     */
    InitGound(clickable, size = 1600) {
        //GROUND
        const gt = new THREE.TextureLoader().load("/assets/resources/texture/terrain/grasslight-big.jpg");
    ...
    SetCalcDistanceMode(dotResourcePath = "assets/resources/geometry/dot.png") {
        ...
        this.surveyObjNum++;
        //클릭된 점에 대한 좌표를 클릭함
        this.threeJsLib.ClickObjectEvent = (obj, pos) => {
            this.surveyTempPointList.push(pos);
            let distance = 0;
            ...
            this.AddPOI(this.surveyLayerName, "POI" + this.surveyObjNum, false, dotResourcePath, pos, 0.01, distance.toFixed(2) + "m", "", "", 0.1, 1000000);
            if (this.surveyCurrentPosition != undefined)
                this.AddLine(this.surveyLayerName, "Line" + this.surveyObjNum, false, [this.surveyCurrentPosition, pos], 100, new THREE.Color(1, 1, 0), 0.01, 1000000);
            this.surveyCurrentPosition = pos;
        };
    }
    /**
    ...
    AddPOI(layerName, objName, clickable = false, imgPath = '/resources/POI/b1.png', position, size = 5, text = "", dataKey, dataValue, minViewDistance = 50, maxViewDistance = 500, returnObj) {
        ...
        if (targetLayer != undefined) {
            ...
            if (this.threeJsLib.AddPOIEvent != undefined)
                this.threeJsLib.AddPOIEvent(objName, POI); //오브젝트가 생긴것을 이벤트 처리
        }
        else {
            //신규레이어생성
    ...
    AddLine(layerName, objName, clickable = false, positions, lineWidth = 1, color = new THREE.Color(1, 1, 0), minViewDistance = 0.1, maxViewDistance = 500) {
        ...
        if (targetLayer != undefined) {
            ...
            if (this.threeJsLib.AddLineEvent != undefined)
                this.threeJsLib.AddLineEvent(objName, newLine); //오브젝트가 생긴것을 이벤트 처리
        }
        else {
            //신규레이어생성
            targetLayer = this.threeJsLib.CreateLayer(layerName);
        ...
        if (targetLayer != undefined) {
            ...
            if (this.threeJsLib.IsExistLayerInObjectName(layerName, objName)) {
                ...
                if (this.threeJsLib.AddObjectEvent != undefined)
                    this.threeJsLib.AddObjectEvent(objName, newMesh); //오브젝트가 생긴것을 이벤트 처리
            }
            /*
            //기존레이어를 가져옴
            let targetLayer = this.threeJsLib.GetLayer(layerName);
            ...
            */
            ...
            if (this.threeJsLib.AddPolygonEvent != undefined)
                this.threeJsLib.AddPolygonEvent(objName, newMesh); //오브젝트가 생긴것을 이벤트 처리
        }
        //거리에 따른 랜더링 기능처리
        targetLayer.visible = true;
        targetLayer.viewDistanceMin = minViewDistance;
